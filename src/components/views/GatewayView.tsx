...
const GatewayView: React.FC = () => {
  ...
  const fetchSettings = () => {
    setLoading(true);
    fetch('http://localhost:3008/api/gateway/config')
      .then(res => res.json())
      .then(data => {
        setSettings(data);
        setLoading(false);
  ...
  if (loading) {
    return (
      <div data-testid="loading-indicator" className="flex-1 flex items-center justify-center bg-[var(--bg-main)]">
        <div className="text-[var(--accent)] text-sm animate-pulse tracking-widest uppercase">Initializing Gateway Interface...</div>
      </div>
    );
  }
  ...
  return (
    <div data-testid="gateway-view-container" className="flex-1 flex flex-col bg-[var(--bg-main)] overflow-hidden">
      <div className="h-16 border-b border-[var(--border-main)] flex items-center justify-between px-8 bg-[var(--bg-surface)]/50 backdrop-blur-md">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 bg-[var(--accent)]/10 rounded flex items-center justify-center text-[var(--accent)]">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>
          </div>
          <h2 className="text-sm font-bold text-[var(--text-bright)] tracking-widest uppercase">Messaging Gateway</h2>
        </div>
        <button 
          onClick={handleSave}
          disabled={saving}
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* PLATFORM LIST */}
        <div className="w-64 border-r border-[var(--border-main)] bg-[var(--bg-sidebar)] flex flex-col">
          <div className="p-4 border-b border-[var(--border-main)]">
            <span className="text-[10px] font-bold text-[var(--text-muted)] tracking-widest uppercase">Neural Channels</span>
          </div>
            ...
            {platforms.map(p => (
              <button
                ...
                className={`w-full flex items-center justify-between px-4 py-4 text-left border-l-2 transition-all ${
                  activePlatform === p 
                    ? 'bg-[var(--bg-surface)] border-[var(--accent)] text-[var(--text-bright)]' 
                    : 'border-transparent text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--bg-sidebar)]'
                }`}
              >
                <div className="flex items-center gap-3">
        ...
        {/* CONFIG AREA */}
        <div className="flex-1 overflow-y-auto p-10 bg-[radial-gradient(circle_at_top_right,var(--bg-sidebar),transparent)]">
          <div className="max-w-2xl mx-auto space-y-10">
            <div className="flex items-center justify-between p-6 bg-[var(--bg-surface)] border border-[var(--border-main)] rounded-xl shadow-2xl">
              <div>
                <h3 className="text-lg font-bold text-[var(--text-bright)] uppercase tracking-tighter flex items-center gap-2">
                  {activePlatform}
                  {settings?.platforms[activePlatform]?.enabled ? 
                    ...
                    <span className="text-[10px] bg-[var(--text-muted)]/10 text-[var(--text-muted)] px-2 py-0.5 rounded-full border border-[var(--text-muted)]/20">OFFLINE</span>
                  }
                </h3>
                <p className="text-xs text-[var(--text-muted)] mt-1">Configure bridge parameters for {activePlatform} neural link.</p>
              </div>
            ...
            <div className={`space-y-6 transition-opacity ${settings?.platforms[activePlatform]?.enabled ? 'opacity-100' : 'opacity-30 pointer-events-none'}`}>
              {(activePlatform === 'telegram' || activePlatform === 'discord' || activePlatform === 'slack') && (
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">Bot Token / API Secret</label>
                  <div className="relative">
                    <input 
                      ...
                      className="w-full bg-[var(--bg-surface)] border border-[var(--border-main)] px-4 py-3 text-sm text-[var(--text-bright)] focus:border-[var(--accent)] outline-none transition-all rounded-lg"
                    />
              ...
              {activePlatform === 'whatsapp' && (
                <div className="p-6 bg-[#00FF41]/5 border border-[#00FF41]/10 rounded-xl space-y-4">
                  <div className="flex gap-4">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-[var(--text-bright)] uppercase">WhatsApp Pairing</h4>
                      <p className="text-xs text-[var(--text-muted)] mt-1 leading-relaxed">
                        WhatsApp uses a multi-device bridge. Once enabled, you'll need to run <code className="text-[var(--accent)] bg-[var(--accent)]/5 px-1 rounded">hermes whatsapp</code> in the terminal to scan the QR code and pair your account.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {activePlatform === 'telegram' && (
                <div className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">Home Channel / Chat ID</label>
                    <input 
                      type="text"
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">Allowed Users (Comma separated)</label>
                    <input 
                      type="text"
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">Reply Mode</label>
                    <select 
                      ...
                      className="w-full bg-[var(--bg-surface)] border border-[var(--border-main)] px-4 py-3 text-sm text-[var(--text-bright)] focus:border-[var(--accent)] outline-none transition-all rounded-lg appearance-none"
                    >
              ...
              <div className="p-6 bg-[var(--bg-surface)] border border-[var(--border-main)] rounded-xl space-y-6">
                <h4 className="text-xs font-bold text-[var(--text-bright)] uppercase tracking-widest border-b border-[var(--border-main)] pb-3">Global Gateway Protocols</h4>
                
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-xs font-bold text-[var(--text-main)]">STT AUTO-TRANSCRIBE</div>
                    <div className="text-[10px] text-[var(--text-muted)] mt-0.5">Automatically transcribe inbound voice messages</div>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input 
                      type="checkbox" 
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-xs font-bold text-[var(--text-main)]">NEURAL ISOLATION</div>
                    <div className="text-[10px] text-[var(--text-muted)] mt-0.5">Isolate group sessions per participant</div>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input 
                      type="checkbox" 
