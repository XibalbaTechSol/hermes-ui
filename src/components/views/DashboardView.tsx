...
const DashboardView: React.FC = () => {
  ...
  useEffect(() => {
    // Fetch system stats
    fetch('http://localhost:3008/api/system')
      .then(res => res.json())
      .then(data => setStats(data))
      .catch(err => console.error(err));

    // Fetch token usage
    fetch('http://localhost:3008/api/usage')
      .then(res => res.json())
    ...
    <div className="flex-1 p-8 overflow-y-auto bg-[#080808]">
      <div className="max-w-7xl mx-auto space-y-10">
          ...
          <div className="flex items-center gap-6">
            <button 
              ...
              className={`px-4 py-2 border text-[10px] font-bold tracking-widest transition-all ${
                customizing 
                ? 'bg-[#FF4D00] border-[#FF4D00] text-[#080808]' 
                : 'border-[#333333] text-[#888888] hover:border-[#FF4D00] hover:text-[#FF4D00]'
              }`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <StatCard label="Memory Load" value={`${stats?.memory?.used || 0}GB`} subValue={`of ${stats?.memory?.total || 0}GB`} color="#FF4D00" customizing={customizing} />
          <StatCard label="CPU Utilization" value={`${stats?.cpu || 0}%`} subValue="Neural tick rate" color="#00FF41" customizing={customizing} />
          <StatCard label="System Uptime" value={`${Math.floor((stats?.uptime || 0) / 3600)}h`} subValue={`${Math.floor(((stats?.uptime || 0) % 3600) / 60)}m active`} color="#007AFF" customizing={customizing} />
          <StatCard label="Neural Token Usage" value={usage?.total_tokens?.toLocaleString() || "0"} subValue={`Est. Cost: $${usage?.total_cost || "0.00"} USD`} color="#FFFFFF" customizing={customizing} />
        </div>

        {/* TOPOLOGY PREVIEW */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-[#111111] border border-[#222222] rounded-xl overflow-hidden h-[500px] relative group">
            ...
            <div className="absolute bottom-6 right-6 flex gap-2">
               <div className="px-3 py-1 bg-[#080808]/80 border border-[#222222] rounded-full text-[8px] font-bold text-[#FF4D00] uppercase tracking-widest">Live View</div>
            </div>
          </div>
          ...
          <div className="bg-[#111111] border border-[#222222] rounded-xl flex flex-col overflow-hidden">
            ...
            <div className="flex-1 bg-[#0c0c0c] p-6 font-mono text-[10px] text-[#555555] overflow-y-auto space-y-3">
               ...
               <div className="text-[#FF4D00]/60 leading-relaxed"><span className="text-[#444444] mr-2">04:21:12</span> [MEMORY] Flush sequence completed.</div>
...
const StatCard: React.FC<{label: string, value: string, subValue: string, color: string, customizing?: boolean}> = ({label, value, subValue, color, customizing}) => (
  <div className="bg-[#111111] border border-[#222222] p-6 rounded-xl hover:border-[#333333] transition-all group relative overflow-hidden">
    ...
    <div className="flex justify-between items-start mb-4">
      ...
      {customizing && <div className="text-[#FF4D00] text-xs font-bold animate-pulse">↓</div>}
    </div>
